...
//---srplab start
bool CFileSystem::addFileArchive(void* memory, s32 len,const io::path& filename, bool ignoreCase, bool ignorePaths, E_FILE_ARCHIVE_TYPE archiveType,
			  const core::stringc& password)
{
	IFileArchive* archive = 0;
	bool ret = false;
	...
	if (archiveType == EFAT_UNKNOWN || archiveType == EFAT_FOLDER)
	{
		...
		if (!archive)
		{
			io::IReadFile* file = createMemoryReadFile(memory,len,filename);
			if (file)
			{
				for (i = 0; i < ArchiveLoader.size(); ++i)
				{
	...
	else
	{
		...
		for (i = 0; i < ArchiveLoader.size(); ++i)
		{
			if (ArchiveLoader[i]->isALoadableFileFormat(archiveType))
			{
				// attempt to open file
				if (!file)
					file = createMemoryReadFile(memory,len,filename);
