...
bool CXMeshFileLoader::parseDataObjectMaterial(video::SMaterial& material)
{
	...
	while(true)
	{
		...
		{
//--------------------------------------------------------------------------------------
//---changed by srplab 2008.01.03			
			// some exporters write "TextureFileName" instead.
			core::stringc TextureFileName,TextureFileName1;
			...
			{
				TextureFileName1=FilePath + FileSystem->getFileBasename(TextureFileName);
				if (FileSystem->existFile(TextureFileName1))
					material.setTexture(textureLayer, SceneManager->getVideoDriver()->getTexture(TextureFileName1));
				// working directory
				else{
					// original name
					material.setTexture(textureLayer, SceneManager->getVideoDriver()->getTexture ( TextureFileName ));
					// mesh path
					if (!material.getTexture(textureLayer))
					{
				...
				// working directory
					if (!material.getTexture(textureLayer)){
						...
						material.setTexture(textureLayer, SceneManager->getVideoDriver()->getTexture(TextureFileName));
						if (material.getTexture(textureLayer))
							os::Printer::log("load x file texture", FileSystem->getFileBasename(TextureFileName).c_str(), ELL_WARNING);
					}else
						os::Printer::log("load x file texture", TextureFileName.c_str(), ELL_WARNING);
				}
			}
			++textureLayer;
			if (textureLayer==2)
...
//---changed by srplab 2008.01.03			
			...
			else
			{
				TextureFileName1=FilePath + FileSystem->getFileBasename(TextureFileName);
				if (FileSystem->existFile(TextureFileName1))
					material.setTexture(1, SceneManager->getVideoDriver()->getTexture(TextureFileName1));
				// working directory
				else{
					// original name
					material.setTexture(1, SceneManager->getVideoDriver()->getTexture ( TextureFileName ));
